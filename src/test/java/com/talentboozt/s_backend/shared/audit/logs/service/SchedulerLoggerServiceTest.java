...
import com.talentboozt.s_backend.shared.audit.logs.repository.mongodb.SchedulerLogRepository;
...
class SchedulerLoggerServiceTest {
    ...
    void logSavesLogWithValidData() {
        ...
        verify(logRepo).save(argThat(savedLog -> savedLog.getJobName().equals("updateTaskProgress") &&
    ...
    void logHandlesNullMessageGracefully() {
        ...
        verify(logRepo).save(argThat(log -> log.getJobName().equals("resetRecurringTasks") &&
    ...
    void logSetsExpirationTimeBasedOnConfiguredDays() {
        ...
        log.setExpireAt(Instant.now().plus(31, ChronoUnit.DAYS)); // Expiration time set to 31 days from the current
                                                                  // time
        when(logRepo.save(any(SchedulerLogModel.class))).thenReturn(log);

        schedulerLoggerService.log("dailyJob", "SUCCESS", "Job ran successfully");

        // Tolerance for comparison: Allow the expiration time to be within 1 second of
        // the expected expiration time
        Instant expectedExpireAt = Instant.now().plus(31, ChronoUnit.DAYS);

        verify(logRepo).save(argThat(savedLog -> savedLog.getJobName().equals("dailyJob") &&
