...
import com.talentboozt.s_backend.shared.audit.logs.repository.mongodb.TaskRewardAuditRepository;
import com.talentboozt.s_backend.shared.audit.logs.service.RewardAuditService;
...
class RewardAuditServiceTest {
    ...
    void recordSavesAuditWithValidData() {
        ...
        rewardAuditService.record(ambassador, task, "COUPON", "reward789", "10% Discount", "ISSUED",
                "Reward issued successfully");

        verify(auditRepo).save(argThat(audit -> audit.getAmbassadorId().equals("ambassador123") &&
    ...
    void recordHandlesNullRewardTitleGracefully() {
        ...
        verify(auditRepo).save(argThat(audit -> audit.getRewardTitle() == null &&
    ...
    void recordSetsExpirationTimeToSevenDays() {
        ...
        rewardAuditService.record(ambassador, task, "SWAG", "reward789", "T-Shirt", "ISSUED",
                "Reward issued successfully");

        verify(auditRepo).save(argThat(audit -> audit.getExpiresAt().isAfter(Instant.now()) &&
    ...
    void recordHandlesNullAmbassadorIdGracefully() {
        ...
        rewardAuditService.record(ambassador, task, "COUPON", "reward789", "10% Discount", "FAILED",
                "Ambassador ID is missing");

        verify(auditRepo).save(argThat(audit -> audit.getAmbassadorId() == null &&
                audit.getStatus().equals("FAILED") &&
    ...
    void recordHandlesNullTaskIdGracefully() {
        ...
        verify(auditRepo).save(argThat(audit -> audit.getTaskId() == null &&
                audit.getStatus().equals("FAILED") &&
