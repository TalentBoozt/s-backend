package com.talentboozt.s_backend.domains.finance.collaboration.service;

import com.talentboozt.s_backend.domains.finance.collaboration.models.CollaborationOperation;
import com.talentboozt.s_backend.domains.finance.collaboration.models.OperationType;
...
public class OperationProcessor {
    ...
    public void processOperation(CollaborationOperation op) {
        ...
        if (applied) {
            ...
            // Only broadcast REFRESH for major state changes
            if (op.getType() == OperationType.APPLY_SCENARIO) {
    ...
    private boolean handleUpdateCell(CollaborationOperation op) {
        ...
        if (path.startsWith("sales.")) {
            ...
            if (plan.getVersion() != null && op.getVersion() != null && op.getVersion() < plan.getVersion()) {
                log.warn("Stale operation rejected: op version {} < entity version {}", op.getVersion(),
                        plan.getVersion());
                return false;
            }
            ...
            if (budget.getVersion() != null && op.getVersion() != null && op.getVersion() < budget.getVersion()) {
                log.warn("Stale budget operation rejected: op version {} < entity version {}", op.getVersion(),
                        budget.getVersion());
                return false;
            }
            ...
            if (model.getVersion() != null && op.getVersion() != null && op.getVersion() < model.getVersion()) {
                log.warn("Stale pricing operation rejected: op version {} < entity version {}", op.getVersion(),
                        model.getVersion());
                return false;
            }
    ...
    private boolean handleApplyScenario(CollaborationOperation op) {
        ...
        // For now we just return true to trigger a broadcast which forces clients to
        // reload
        return true;
    }
